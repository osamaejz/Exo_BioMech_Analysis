clearvars -except Uphill_Exo_Biomechanics_Data
% load ("Uphill_Data/Uphill_Exo_Biomechanics_Data.mat")
...
extractHipAngles = @(C) cellfun(@(T) ...
    T{:, ...
      find(contains(T.Properties.VariableNames,'RHipAngles'),1) : ...
      find(contains(T.Properties.VariableNames,'RHipAngles'),1) + 2}, ...
    C, 'UniformOutput', false);

extractKneeAngles = @(C) cellfun(@(T) ...
    T{:, ...
      find(contains(T.Properties.VariableNames,'RKneeAngles'),1) : ...
      find(contains(T.Properties.VariableNames,'RKneeAngles'),1) + 2}, ...
    C, 'UniformOutput', false);

extractAnkleAngles = @(C) cellfun(@(T) ...
    T{:, ...
      find(contains(T.Properties.VariableNames,'RAnkleAngles'),1) : ...
      find(contains(T.Properties.VariableNames,'RAnkleAngles'),1) + 2}, ...
    C, 'UniformOutput', false);

for subj = 1:length(Uphill_Exo_Biomechanics_Data)

    NENL_kin = Uphill_Exo_Biomechanics_Data{subj}.R_gait_cycles_kin;
    Load_kin = Uphill_Exo_Biomechanics_Data{subj}.R_Load_gait_cycles_kin;
    Exo_kin = Uphill_Exo_Biomechanics_Data{subj}.R_Exo_gait_cycles_kin;
    ExoLoad_kin = Uphill_Exo_Biomechanics_Data{subj}.R_Exoload_gait_cycles_kin;
